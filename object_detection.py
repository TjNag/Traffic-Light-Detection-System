# Description: This program helps detect objects (e.g. traffic lights) in images.
...
def save_image_annotated(img_rgb, file_name, output, model_traffic_lights=None):
  ...
  output_file = file_name.replace('.png', '_test.png')
  ...
  for idx in range(len(output['boxes'])):
    ...
    if obj_class == LABEL_TRAFFIC_LIGHT:
      ...
      if model_traffic_lights:
        ...
        #cv2.imwrite(output_file.replace('.png', '_crop.png'), cv2.cvtColor(img_inception, cv2.COLOR_RGB2BGR))
